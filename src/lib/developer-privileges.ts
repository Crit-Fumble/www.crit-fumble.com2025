 ...
 * @returns true if the user has developer privileges
 */
export function isDeveloper(user: CritUser | null | undefined): boolean {
  ...
  return (
    (user as any).verifiedPhone === devPhone &&
    (user as any).verifiedEmail === devEmail &&
    (user as any).verifiedDiscord === devDiscord
  );
}
