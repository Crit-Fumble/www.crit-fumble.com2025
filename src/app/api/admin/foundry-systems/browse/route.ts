...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get currently enabled systems from RpgSystem
    const enabledSystems = await prismaMain.rpgSystem.findMany({
      where: { deletedAt: null, isEnabled: true },
      select: { systemId: true },
    const enabledSystemIds = new Set(enabledSystems.map((s: { systemId: string }) => s.systemId))
