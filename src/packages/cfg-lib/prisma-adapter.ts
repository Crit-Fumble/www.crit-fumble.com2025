...
/**
 * Custom Prisma Adapter wrapper that adds session activity tracking
 */
  ...
  return {
    ...
    async linkAccount(account) {
      ...
      if (account.provider === 'discord') {
        // Discord provides these fields in the profile
      ...
      // Update CritUser login timestamp and DEPRECATED provider fields
      // (Keep updating deprecated fields for backward compatibility during migration)
      try {
    ...
    async createSession(session) {
      const createdSession = await prisma.session.create({
        data: {
          ...
          // IP and UserAgent will be added via middleware/API route
        },
      })
      ...
      try {
        await prisma.critSessionLog.create({
          data: {
            ...
            sessionToken: session.sessionToken,
            loginMethod: 'oauth', // We don't have provider info here
    ...
    // This is critical for session validation to work with our Player model
    async getSessionAndUser(sessionToken) {
      ...
      // This significantly reduces database writes while still tracking activity
      const now = new Date()
      ...
      return {
        ...
        user: {
          ...
          email: player.email || '',
          emailVerified: null, // Player model doesn't have this field
          profileCompleted: player.profileCompleted, // Add to session for proxy
        },
      }
    },
  }
